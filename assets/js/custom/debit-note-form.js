...
$(document).ready(function(){
		...
		if (ref_id != "" || ref_id != 0) {
			$.ajax({
				...
				success: function (response) {
					...
					$('#modal-xl .modal-title').html("Carete Credit Note [ Party Name : "+party_name+" ]");
    ...
    $(document).on("change",'#order_type',function(){
        ...
        if(order_type == "Purchase Return"){
			...
			$("#itemDiv").removeClass("col-md-12").addClass("col-md-8");
			$("#batchDiv").removeClass("hidden");
        }else{
            $('#itemForm #stock_eff').val("0");
			$("#itemDiv").removeClass("col-md-8").addClass("col-md-12");
