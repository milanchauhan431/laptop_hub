    ...
    <div class="col-md-12">
        <div class="row">
            ...
            <div class="col-md-3 form-group">
                ...
                <input type="text" name="qty" id="qty" class="form-control numericOnly req" value="<?=(!empty($dataRow->qty))?$dataRow->qty:''?>" readonly >
            </div>
...
$(document).ready(function(){
        ...
        if(item_id){
            $.ajax({
                ...
                success:function(res){
                    $("#batch_no").html(res.batchOption);
                    $("#batch_no").select2();
                }
            });
        }else{
            $("#batch_no").html('<option value="">Select</option>');
            $("#batch_no").select2();
        }
    //});
    ...
    $(document).on('change','#batch_no',function(){
        ...
        $("#purchase_price").val("");
        $("#unique_id").val("");
        $("#itemKitData").html('<tr><td class="text-center" colspan="6">No data available in table</td></tr>');
        if(batch_no){
            $("#purchase_price").val($(this).find(":selected").data('price'));
            $("#unique_id").val($(this).find(":selected").data('unique_id'));

            $.ajax({
                url : base_url + controller + '/getItemKitForCustomization',
                type : "post",
                data : {mir_trans_id : $(this).find(':selected').data('mir_trans_id')},
                dataType : 'json',
                success : function (res){
                    $("#itemKitData").html("");
                    $("#itemKitData").html(res.kitHtml);
                }
            });
        }
    });
...
/* function calculatePartAmount(){
	...
	$("#part_amount").html(amountSum.toFixed(2));
} */
</script>